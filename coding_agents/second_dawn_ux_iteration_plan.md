...
### September 19 — turn visibility and fewer end-of-turn interruptions
Implemented the follow-up slice in `second_dawn_turn_order.md`: fair live initial starter, clockwise roster, faction-marked ambassador offer, toolbar sector controls, consolidated fleet inspection, passed/reaction-only guidance and saved auto-pass-unless-attacked. Auto-pass introduces an authoritative turn boundary so consecutive same-owner turns receive fresh clocks and AI budgets. Later user feedback added automatic best legal reputation awards with private brief feedback/reopen, plus expiring combat results. Detailed rules, preservation tests, browser images and independent scheduling review are in the linked feature documents. Main-only release gates and hosted verification follow the final combined run; physical-device/user playtests remain separate from engineering browser checks.
...
- Optional Minor Species: legal four-tile recruitment market, explicit population choice and private reputation-return acknowledgement, upkeep funding warning, acquired public cards, correct discounted research/build previews and separate scoring bonuses. Eleven new UI cases and related regression batches passed; desktop/mobile renders and real-engine purchases verified. [Frontend evidence](second_dawn_minor_species_frontend.md).

### September 20 — Rules choices, discovery reference, and compact resource header
Shared padded radio cards clarify rules selection in solo/room setup. Less Random Command Center now exposes all 27 canonical discovery types with search, effects, part stats, public stock and depleted options. Desktop resource symbols carry accessible stock/income labels; constrained faction-clock text prevents collisions. Sixty bounded tests, lint/build and desktop/mobile browser review pass. [Delivery evidence](second_dawn_ux_clarity.md). Human newcomer/expert playtests remain pending; no rules or deployment changes.

### September 20 — Independent custom rules and upkeep scrolling
Players can choose 1–20 rounds and independently configure open technology, public discoveries/reputation, exploration, combat Jokers, variant inventories/developments and faction rules, or restore either complete preset. Additive saved settings preserve old matches; AI, projections, score privacy and round labels follow effective rules. Hidden upkeep maps no longer prevent Command Center scrolling. [Release evidence](custom_game_rules_release.md) links failing-first, mixed-configuration, complete AI-match and desktop/mobile checks; deploy follows passing gates per user preference. Human playtests remain pending.

### September 20 — Proactive confirmation for complete plans
- Outcome: full, valid upgrade drafts, fully placed build orders, and complete movement routes raise the same centered attention style as turn/upkeep notices, with explicit confirmation and Keep editing.
- Rules preserved: no automatic submission; partial/invalid/stale/blocked drafts stay quiet; queued movement waits for Queue route; funding choices remain explicit; material costs, upkeep, conversions, and betrayal consequences remain visible. Ordinary committed actions still use existing automatic advancement; paid/faction continuations are unchanged.
- Dismissal survives workspace remounts for the same plan within the current match/seat/turn. New plan or turn can prompt again; hidden tabs and settings/history/recap/inspection/pending decisions suppress prompts.
- Validation: 110 targeted tests across 15 suites passed, including new failing-first upgrade/build/move tests and existing turn attention, planner, draft recovery, upgrade legality, and automatic advancement regressions. Desktop and 390×844 mobile browser checks showed focused explicit Apply, no spontaneous submission, no overflow, and successful explicit submission. Newcomer criterion: final confirmation is discoverable without scrolling; expert criterion: dismiss and continue editing without losing the draft or repeated interruptions.
- Decision log: readiness is evaluated in each planner; shared dialog and turn-scoped session acknowledgments do not change save schema or engine state. Full allowance is the unambiguous completion trigger; smaller plans keep their existing explicit confirmation buttons.
- Follow-ups: human playtest prompt frequency during multi-class fitting. Release uses the standing lint/test/build then main deployment policy.

### September 20 — Spatial decisions on the main galaxy
- Outcome: sector placement/rotation, influence control decisions, upkeep abandonment, warp portal placement, and automatic colonization use the existing main galaxy with desktop inspector/mobile sheet. Their live board flow no longer renders a second galaxy or a separate full-screen placement workspace.
- Decisions: preview view/highlights are published through a typed decision-map context; authoritative state stays immutable. Exploration and control initially focus their target; bankruptcy and portal placement require explicit selection. Existing pending decision components remain mounted while browsing so rotation, choices, and allocations survive. Reward and battle decisions retain their appropriate popup presentations.
- Experience: desktop keeps the map interactive alongside controls; mobile portrait leaves map space above the scrollable sheet and landscape places controls beside it. Existing minimize/return, camera controls, sector details, cost previews, and confirmations remain available. Newcomer criterion: select directly on the visible map and see the consequence next to it; expert criterion: inspect and return without losing a choice or orientation.
- Validation: 95 tests across 17 bounded suites passed, including failing-first shared-map exploration/economy/integration tests; existing choice preservation, mobile navigation, upkeep scrolling, action draft, attention, and colonization regressions passed. Lint and production build passed. Browser checks covered desktop exploration/upkeep selection, mobile portrait/landscape rotation and legal gating, exactly one galaxy, no horizontal overflow, and no console errors.
- Risks/rollback: UI-only release, no backend/schema changes. Revert this slice if needed. Preview data is scoped to the current decision ID and never submitted without the existing explicit resolve command. Read-only history/score inspection and combat presentation remain separate; human playtests can assess further consolidation.

### September 24 — Public room spectators
Room-link visitors can watch human and AI activity, pause automatic focus by inspecting the galaxy, and resume following. Sectors, fleets, science/research tracks, public command centers, blueprints, history and standings share guarded public presentation. Dedicated server projections omit private holdings/choices and management metadata; spectators never receive an owned seat or mutation controls. Anonymous viewing does not create a guest, and returning players can still sign in. See [spectator delivery evidence](second_dawn_spectators.md) for privacy tests, regression gates, reviewed desktop/mobile screenshots, release status and human playtest follow-up. Seat takeover and puzzles remain out of scope.

### September 25 — Reddit action layout and optional pass-order turns
The approved action layout moves six ordinary actions from the desktop header into a fixed rail, with Colonize/Trade beneath and a turn control at the bottom. Mobile groups the same choices in its action sheet and gives Pass a fixed footer position beside Choose action; after passing, auto-pass occupies that turn area. Existing mobile navigation remains on the second footer row. A separate creation-rules checkbox lets hosts use first-pass order for the next round; omitted settings preserve clockwise turns and the first passer still gains 2 money. Thirty-nine focused tests across nine suites, lint/build, and desktop/mobile local-browser checks pass. Human newcomer/expert action-search playtests remain to be done; release evidence follows after integration.

### September 25 — Integrated release candidate
The approved action rail/mobile footer and optional pass-order rule are integrated with Exiles and Lyra in the new `expanded-v2` collection. The option appears in the game-creation rule checkboxes and room editor; after Pass, auto-pass uses the fixed turn area. The new collection is the default for new games. Existing base and `expanded-v1` games keep their pinned rosters. The integrated picker test first exposed a v1 roster leak, then passed after explicit profile filtering. Bounded gameplay, settings, room, and AI suites pass; final release gates and live verification remain to be recorded. Human newcomer/expert playtests remain outstanding.

Release verification: `c8dd0d5` merged to `main` and pushed. The existing Convex dev deployment reported functions ready before the frontend release. Vercel production deployment `dpl_CRzPCXGSKG7HrGiFkZmeJjEZ5BtT` reached Ready and took the public alias. On the public site, the solo creation form showed the pass-order checkbox unchecked by default, let it be checked, and selected `expanded-v2` with Exiles and Lyra visible. The mobile multiplayer creation form also showed the checkbox and selected collection, with no horizontal overflow or page errors. Final integrated lint and build passed; relevant faction/profile, layout, rules/Convex, and bounded AI suites passed. Human playtests remain outstanding.

### September 25 — Affordable-action shortcut to economy tracks
The count in the desktop and mobile game header now opens the player’s own Command Center and scrolls to Income & upkeep tracks. The neighboring “This round” disclosure still explains the forecast assumptions. The change submits no game command and preserves drafts. Its new desktop/mobile navigation tests failed first, then 23 bounded UI tests, lint and build passed. Local Chromium checks on both widths confirmed the track is in view, with no horizontal overflow or page errors. Human playtest confirmation remains pending.
Release verification: `fbafec4` reached `main` and Vercel production deployment `dpl_AmvnQ8CNfLkwtRcFhy27fkt9cjrz` became Ready with the public alias. A live guest game’s affordable-action button opened the Income & upkeep tracks without page errors. The final deployed build is the same UI-only commit that passed the local desktop/mobile scroll checks.

### September 26 — Declined AI diplomacy and clearer response
The incoming ambassador offer now starts with neither answer selected, explains that declining closes the request, and requires an explicit “Decline and continue” or “Accept ambassadors” confirmation. The proposer’s post-combat finish choice says there will be no more offers this round. AI records a rejection for the current round and filters further action-turn offers to that player; human offers remain legal. Hard-AI sampled worlds preserve that proposer-only rejection memory, and the record resets with the next round. The failing-first engine/UI regressions and 78 focused AI, diplomacy, protocol and draft tests passed. After restarting in an unrestricted workspace, 74 relevant tests across eight suites, lint, and build passed. The established Convex dev deployment reported functions ready; a local Playwright home-page check found no page errors or Vite overlay.
Release verification: `7164b8b` reached `main` and Vercel production deployment `dpl_7KkZ4bpQcN8MRGnjirupjG6dj3aB` became Ready with the public alias. GitHub's Vercel status succeeded. Desktop and mobile public-page checks found no page errors, Vite overlay, or horizontal overflow. The live decline flow awaits a player session.

### September 26 — Base and Expanded faction collections
New solo and room creation now present two collection choices: Base and Expanded. Expanded is the complete eighteen-civilization collection, including Exiles and Lyra. Existing `expanded-v1` saves and rooms retain their original roster; an old room editor explains the legacy selection and upgrades only when the host explicitly selects Expanded. Two picker tests failed first, then 21 focused picker, room, and profile tests, lint, and build passed. Local desktop and mobile new-game forms showed both choices with no page errors or horizontal overflow.
Release verification: `b0f29e8` reached `main`, and Vercel deployment `dpl_98kKd7E6HtgBCzdkYejT4k8uC1DX` became Ready with the public alias. The live site showed the two choices in solo creation at desktop and mobile widths and in room creation at desktop width, with Expanded selected and no page errors or horizontal overflow.

### September 26 — Desktop action rail on the right
After reviewing a local midgame screenshot, the user accepted galaxy → inspector → fixed action rail on desktop. The rail remains at the right edge when the inspector is hidden and when Research fills the main area. History, Settings, and Hide sector details remained accessible in local browser checks. Mobile keeps its footer layout. The desktop DOM order now matches the visual order for keyboard traversal. Fifteen focused desktop/mobile tests, lint, and build passed during the local review; final release verification follows. See [the screenshot review](action_rail_right_local_review.md).
### September 26 — Consolidated creation rule options
New Standard setup checks Minor Species and estimated combat odds by default. Their switches and the warp portal and Rift Cannon switches appear under Individual rule options for solo and room creation. Saved rooms with omitted legacy settings remain off. Failing-first UI tests cover section placement, preset switching, and solo defaults; final validation and release evidence follow.
Validation: 26 focused tests passed, including legacy room persistence, followed by lint and build. Local desktop and 390px mobile solo setup and desktop room creation showed the four options together, both new defaults on, no page errors, and no horizontal overflow. Human usability review remains open.
Release verification: `d1470fc` reached `main`; Vercel deployment `dpl_8XS1eDsc9GwJGz3pY7YBy26nNDdA` became Ready with the public alias. Live solo creation at desktop and mobile widths and room creation at desktop width showed both new defaults on, all four options together, no page errors, and no horizontal overflow.
...
Release verification: `daa876c` reached `main`; Vercel deployment `dpl_G6yEhMevkidEDJSJ5xh6jEwgZjyA` became Ready with the public alias. Local desktop/mobile Research checks showed the final layout. A live game-start recovery overlay prevented a second public Research navigation check.

### September 26 — Combat reputation presentation
Standard battle reputation remains an automatic private settlement, but its result now follows the combat playback in the combat results dialog rather than appearing in a separate header row. Public-reputation choices follow the active battle overview inside its choice dialog. Failing-first UI tests cover both placements; final validation and release evidence follow.
Validation: 29 focused combat/reputation tests passed. Local desktop and mobile combat-result fixtures showed the result after battle impacts inside one dialog, with no page errors or horizontal overflow. The final combined gate passed 49 focused tests, lint, and build. Human handoff playtest remains open.
Release verification: `af8111e` reached `main`; production deployment `dpl_5YSFVfTip2gCCbvAXuaCkvXHRtnS` became Ready with the public alias. Live desktop and mobile home checks showed no page errors or overflow. The combat-specific interaction was verified locally; a live human battle handoff playtest remains.

### September 26 — Research discounts within technology slots
After reviewing the first card-row layout, the user asked to remove its separate discount card. Each track now has exactly seven positions: researched technology cards fill them, and empty positions show their effective science discount as the printed adjustment. The current discount sits beside the track name, such as “Nano · Discount 4”; the slot-number labels are gone. Research pricing and inspection remain unchanged. Failing-first Research and Command Center tests cover both layouts and Minor Species bonuses; final validation and release evidence follow.
Validation: 20 focused research/empire/Minor Species tests passed. Local desktop and mobile fixtures showed seven positions, the adjacent heading discount, no page errors, and no page overflow. The final combined gate passed 49 focused tests, lint, and build. Human comprehension review remains open.
Release verification: the same `af8111e` deployment reached Ready. Local deterministic Research views verified the final slot layout, while live desktop and mobile home checks found no page errors or overflow. Human comprehension review remains open.

### September 26 — Research VP on the technology grid
Each of the seven spaces now shows both its science discount and the cumulative track score reached by filling it: 0, 0, 0, 1, 2, 3, and 5 VP. The markers remain visible after a technology fills a space, so players can compare future discounts and scoring thresholds in one row. The track heading still shows the effective current discount. Failing-first Research and Command Center tests cover empty and filled spaces using the authoritative scoring function; validation and release evidence follow.
Validation: 36 focused tests, lint, and build passed. Local Chromium at desktop and mobile widths showed the two markers on empty and filled spaces, no page errors, and no horizontal overflow. Human review of score legibility remains open.
Release verification: `0d648bd` reached `main`; production deployment `dpl_J3ALzf16363n8x9AUDQmDQ9o4ykn` became Ready with the public alias. Live desktop and mobile home checks showed no page errors or overflow. Research-specific visuals were verified in local deterministic fixtures.

### September 26 — Dice animation continuity
Skipping a volley now applies to that roll only; the next opponent volley can animate in the same mounted combat view. The game-level Animations setting is the single motion control: OS reduced motion sets the default, while an explicit in-game “on” choice enables the 3D throw. Tests first reproduced both failures, then 58 focused dice, sound, combat and board tests passed. Local WebGL2 browser checks at normal and reduced-motion preferences showed the first and next throws, including after Skip, without page errors. Final gates and release verification follow.
Release verification: `9c8f607` reached `main`; production deployment `dpl_4Mg1yyTJKTrzpB1o7uncMocnj8H2` became Ready with the public alias. Live desktop and mobile home checks found no page errors or overflow. The repeated-roll interaction was verified in a local deterministic browser fixture; live human combat remains a follow-up.
