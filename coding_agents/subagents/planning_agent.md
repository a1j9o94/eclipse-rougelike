...
Release: `7164b8b` was fast-forwarded into `main` and pushed after the Convex update. Vercel production deployment `dpl_7KkZ4bpQcN8MRGnjirupjG6dj3aB` reached Ready, took the public alias, and GitHub reported success. Desktop and mobile public-page checks loaded with no page errors, overlay, or horizontal overflow. A live decline-to-next-round playtest remains open.

## 2026-09-26 — Two faction collection choices
Outcome: new solo and room creation show only Base and Expanded, with Expanded containing all eighteen civilizations including Exiles and Lyra.
Acceptance: the two visible choices map to `base` and `expanded-v2`; existing `expanded-v1` games and rooms retain their pinned roster; editing an old room explains its legacy collection and requires an explicit Expanded click to upgrade it.
Risks & rollback: a hidden `expanded-v1` value could be silently upgraded or mislabeled. Keep the versioned engine profile and server validators intact; revert the picker/label change if editing legacy rooms becomes confusing.
Tests (failed first): picker has exactly two buttons, choosing Expanded emits `expanded-v2`, and legacy room presentation preserves `expanded-v1` until clicked. Run room/profile adjacent suites, lint, build, and browser checks.
Decision Log: collapse only the creation UI choices. `expanded-v1` remains a storage compatibility value, while the visible Expanded option always means the complete v2 roster for new games.
Follow-ups: record tests, deployment, and live creation-page verification.
Result & Next Steps: the picker shows only Base and Expanded; Expanded selects the complete `expanded-v2` roster. Existing `expanded-v1` rooms keep their pinned roster and show an explicit explanation until the host selects Expanded. Two new UI tests failed first, then 21 focused picker/room/profile tests passed. Lint and build passed. Local Playwright checks of the New game form at 1365px and 390px showed the two cards, Expanded selected, no page errors or overflow.
Release: `b0f29e8` reached `main`; Vercel production deployment `dpl_98kKd7E6HtgBCzdkYejT4k8uC1DX` became Ready with the public alias. Live solo creation at desktop/mobile widths and room creation at desktop width showed only Expanded and Base, with Expanded selected and no page errors or horizontal overflow.

## 2026-09-26 — Local right-side action rail review
Outcome: let the user compare a desktop layout with the galaxy on the left, sector/action inspector next, and the fixed action rail on the far right.
Acceptance: desktop rail follows the inspector visually and in keyboard order; sector inspection, action selection, and turn controls stay available; collapsed-inspector and full-map modes keep the rail at the right edge. Mobile stays unchanged. Provide local screenshots for review, without merging or deploying.
Risks & rollback: narrower right-side workspaces may feel crowded, and grid-mode overrides could place the rail off-screen. Keep the change in a local branch; revert the CSS and DOM order if the review rejects it.
Tests (must fail first): desktop chrome DOM order; browser geometry at normal and collapsed inspector widths, plus local screenshot review. Run adjacent desktop/mobile board tests, lint, and build.
Decision Log: put main content before the inspector and rail in DOM order so keyboard traversal matches the proposed left-to-right desktop layout. Preserve existing rail width and independent inspector scrolling for an apples-to-apples review.
Follow-ups: user accepted the screenshot review; remove the temporary preview entry, rerun gates, release, and verify the public game.
Result & Next Steps: the desktop DOM and grid now read galaxy → inspector → 112px action rail. The rail stays pinned at the right edge with the inspector open, collapsed, or replaced by a full-width Research workspace; mobile retains its footer and has no rail. The failing-first desktop chrome test passed, and 15 related desktop/mobile tests passed after correcting a stale mobile expectation that still looked for Pass inside the action picker. Lint and build passed. The isolated local midgame fixture showed no page errors or horizontal overflow at 1440px; the mobile fixture at 390px also showed no page errors or overflow. User accepted the layout. History opened in the inspector, Settings opened its dialog, and Hide sector details expanded the map without moving the rail. The temporary preview entry has been removed before release.
## 2026-09-26 — Consolidated individual setup options
Outcome: new Standard solo and room games offer Minor Species and combat estimates by default, with warp portals and Rift Cannons in the same Individual rule options list.
Acceptance: all four choices appear in one fieldset in solo and room setup; Standard starts with Minor Species and estimates checked, and players can turn either off; preset switching resets them consistently; existing room settings without those fields keep their saved off state.
Risks & rollback: defaulting omitted legacy room fields on would silently change old rooms. Keep backend defaults and loaded room values unchanged; revert this UI-only branch if setup choices do not persist correctly.
Test list (failed first): shared rules fieldset location, Standard/preset behavior, and solo default/override. Run focused room and rules settings suites, lint, build, and browser smoke.
Decision Log: treat Minor Species and combat estimates as room/setup settings in the shared rules component, not engine rule options. Only new creation state gets explicit true defaults; legacy omitted values still display false. Remove the separate Optional components fieldset while keeping its two checkboxes.
Follow-ups: verify new room creation and mobile layout in browser, then release after gates and check production.
Result & Next Steps: 26 focused setup, room, and legacy persistence tests passed after the three UI tests failed first. Lint and build passed. Local Playwright checks at 1366px and 390px showed four options in one fieldset, new Standard defaults on, no optional-components fieldset, no page errors, and no horizontal overflow. Room creation also showed both new defaults on. Release verification follows.
## 2026-09-26 — Exiles Orbital labels and build clarity
Outcome: Exiles players see Orbital across shipyard, fleet, movement and combat menus, and Build explains how the Orbital blueprint becomes a defender.
Acceptance: Exiles Build offers Orbital without an unusable Starbase card; the Orbital card lists its current printed/installed blueprint parts and says colonization activates its defender. Shipyard and empire fleet labels say Orbital; ordinary factions retain Starbase. No engine state, component identity, or battle rules change.
Risks & rollback: a global rename would confuse non-Exiles Starbases, and a Build UI change could imply the Orbital fights before colonization. Use faction-aware labels and explicit activation copy; revert UI changes if either fails.
Test list (failed first): Exiles shipyard labels, empire fleet labels, and Build card parts/activation explanation. Run adjacent Exiles engine and board/room UI suites, lint, build, browser check.
...
## 2026-09-26 — Research VP in every technology slot
Outcome: the research grid shows each slot's science discount and cumulative research VP together, whether the slot is empty or filled by a technology.
Acceptance: seven positions per track show the existing discounts and official cumulative VP thresholds (0, 0, 0, 1, 2, 3, 5); filled technologies remain inspectable; the current-discount heading remains beside each track name. Research and Command Center agree, and research prices/scoring do not change.
Risks & rollback: extra slot text may crowd mobile cards or suggest VP is earned separately per technology. Label VP as the track total at that slot and retain horizontal track scrolling; revert this display-only change if it obscures technology effects.
Test list (must fail first): empty slot shows its discount and resulting track VP; filled slot retains the same markers and technology inspection; both Research and Command Center show official thresholds. Run focused research/empire/scoring suites, lint, build, and desktop/mobile visual checks.
Decision Log: use the authoritative `researchTrackVp` scoring function for the value after filling each position. Keep discount derived from the same current slot index as before; display both as per-position board markers, not a separate card or additive VP reward.
Follow-ups: human review of mobile legibility and scoring comprehension; production verification after release.
Result & Next Steps: three grid tests failed first, then 36 focused Research, Command Center, Minor Species and scoring tests passed. Local Chromium at 1366px and 390px showed seven slots, both markers on filled and empty spaces, no page errors, and no page overflow. React component review found no state or hook issues; lint and build passed. Release and human scoring-comprehension review remain.
Release: `0d648bd` reached `main`; production deployment `dpl_J3ALzf16363n8x9AUDQmDQ9o4ykn` became Ready with the public alias. Live desktop and mobile home checks found no page errors or overflow. The Research-specific visuals were checked locally; human scoring-comprehension review remains.

## 2026-09-26 — Consistent combat dice animation
Outcome: skipping one combat volley affects that roll only; the next opponent volley animates when motion and 3D dice are enabled.
Acceptance: a new roll identity clears the Skip state without replaying already-presented dice; the current roll still becomes instantly inspectable after Skip; no game state or RNG changes. Respect explicit motion and reduced-motion settings.
Risks & rollback: resetting Skip on unrelated rerenders could replay a roll. Key the skip state to the authoritative dice identity, and retain the existing one-throw guard; revert UI-only behavior if a replay occurs.
Test list (must fail first): skip a volley, deliver a different roll in the same mounted playback, and expect 3D enabled again. Run dice integration/renderer/sound and combat flow suites, lint, build, and browser playback checks.
Decision Log: audit found `CombatPlayback`'s unkeyed `skipMotion` state persists for its whole mount. Scope Skip to the current `opponentRollId`, which includes battle and die identities. A second failing-first test showed that `DiceRoll3D` and its CSS independently overrode an explicit in-game Animations-on choice when the OS prefers reduced motion. Apply the OS preference once as the game's default, then let the user's in-game choice control the renderer. Keep the engine and authoritative RNG untouched.
Follow-ups: verify repeated volleys in a live game and evaluate clarity of reduced-motion behavior.
Result & Next Steps: the Skip and reduced-motion override tests failed first, then 58 focused dice, sound, combat and board tests passed. Local Chromium with WebGL2 at normal and reduced-motion preferences showed a visible overlay for the first roll and for the next roll after Skip, with no page errors. Reduced motion remains the board's default until explicitly overridden; no engine or RNG code changed. Lint/build and release verification follow.
Release: `9c8f607` reached `main`; production deployment `dpl_4Mg1yyTJKTrzpB1o7uncMocnj8H2` became Ready with the public alias. Live desktop and mobile home checks found no page errors or overflow. The repeated-roll interaction was verified locally with WebGL2; a live combat session remains a follow-up.
