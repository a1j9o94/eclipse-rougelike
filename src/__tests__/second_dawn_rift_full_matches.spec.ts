...
describe("complete bounded Rift expansion matches", () => {
  it.each([2, 3, 4, 5, 6])(
    ...
    (count) => {
      ...
      // Ensure the new rare is offered in this deterministic scenario while conserving its singleton.
      state.supplies.technology = state.supplies.technology.filter(id => id !== 'rift-cannon');
      if (!state.technologyMarket.includes('rift-cannon')) state.technologyMarket.push('rift-cannon');
      let commands = 0;
