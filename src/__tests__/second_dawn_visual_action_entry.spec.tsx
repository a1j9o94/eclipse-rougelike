...
it('opens the embedded build-first workspace without preselecting the inspected sector',()=>{const view=fixture();const home=view.sectors.find(s=>s.owner==='a')!;render(<SecondDawnBoard view={view} candidates={legalCommands(view)} initialSectorId={home.id} connected busy={false} status="" onSubmit={vi.fn()} onMenu={vi.fn()}/>);fireEvent.click(screen.getByRole('button',{name:'Build',exact:true}));expect(screen.getByRole('region',{name:'Assemble your build order'})).toBeVisible();fireEvent.click(screen.getByRole('button',{name:'Add interceptor'}));expect(screen.getByRole('button',{name:/1. Interceptor Unplaced/})).toBeVisible();expect(screen.queryByRole('combobox',{name:'Choose an option'})).toBeNull();});
