...
it('labels a passed player as reactions only during the action phase and explains the limits',()=>{const v=view();v.phase='action';v.seats[0].passed=true;const rendered=render(<UpkeepSummary view={v}/>);const summary=rendered.container.querySelector('summary')!;expect(within(summary).getByText('Reactions only')).toBeInTheDocument();expect(summary).toHaveAttribute('title',expect.stringContaining('Upgrade, Build or Move'));fireEvent.click(summary);expect(screen.getByText(/one activation, without an influence disc/)).toBeVisible();expect(screen.getByText(/Resource costs, available pieces and blueprint slots still apply/)).toBeVisible();});
