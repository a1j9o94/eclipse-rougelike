...
export default function UpkeepSummary({view}:{view:PlayerView}){
 ...
 const reactionExplanation='Upgrade, Build or Move: one activation, using one influence disc. Resource costs, available pieces and blueprint slots still apply.';
 ...
 const nextActionLabel=view.phase==='action'&&!f.eliminated&&f.nextUpkeep!==null?`${reactionsOnly?(progress?'Next new reaction':'Next reaction'):(progress?'Next new action':'Next action')}: ${f.nextUpkeep} upkeep · ${balanceLabel(f.nextBalance!)}`:null;
 ...
 return <details className="dg-upkeep-summary">
  ...
  <div className="dg-upkeep-explanation"><h3>{reactionsOnly?'Reaction turns':'Affordable-action forecast'}</h3>{reactionsOnly&&<p>{reactionExplanation}</p>}<p aria-label={`Current upkeep: ${f.upkeep} money; ${f.balance<0?`${-f.balance} money short`:`${f.balance} money left`} at round end`}><span>Round-end upkeep</span> · {f.upkeep} money · {balanceLabel(f.balance)}</p>{nextActionLabel&&<p className={f.nextBalance!==null&&f.nextBalance<0?'dg-danger':''}>{nextActionLabel}</p>}<p>{f.money} money + {f.income} income − {f.upkeep} upkeep = {balanceLabel(f.balance)}</p><p>{f.influenceDiscs} influence discs remain. Starting an action or reaction moves one disc off your track; the newly uncovered number is your round-end upkeep bill, not an immediate money payment.</p>{progress&&<p>Your remaining {progress.action[0].toUpperCase()+progress.action.slice(1)} activation uses no additional disc.</p>}<p>Forecast assumes no further trading, direct spending, territory or income changes, or special effects.</p><p>Taking control of a sector also uses a disc. Research and construction have separate science or material costs. A shortfall explains risk; it does not block a legal action.</p></div>
 </details>;
}
