...
export default function UpkeepSummary({view}:{view:PlayerView}){
 ...
 const compactLabel=capacity.status==='affordable'?`${capacity.actions} ${capacity.actions===1?'action':'actions'} affordable`:capacity.status==='unfunded'?`${capacity.shortfall} money short`:capacity.status==='no-discs'?'No discs left':capacity.status==='passed'?'Passed':'Eliminated';
 ...
 const hoverBreakdown=`${capacityLabel}. ${f.influenceDiscs} influence discs · ${f.money} money + ${f.income} income − ${f.upkeep} upkeep = ${balanceLabel(f.balance)}. ${nextActionLabel??''}`;
