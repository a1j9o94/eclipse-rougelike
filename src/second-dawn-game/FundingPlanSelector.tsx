...
export default function FundingPlanSelector({view,command,disabled,onChange}:{view:PlayerView;command:FundedCommand;disabled:boolean;onChange:(command:FundedCommand)=>void}){
 ...
 const payment=(trade:FundedCommand['trades'][number])=>tradeQuote(seat.faction,trade.from,trade.to,trade.amount)?.input??0;
 ...
 return <section className="dg-funding" aria-label="Purchase conversion">
  ...
  <small>{[...new Set(command.trades.flatMap(t=>tradeRates(seat.faction,t.from,t.to).map(rate=>`${rate.input} : ${rate.output}`)))].join(' / ')} exchange</small><p className={balance<0?"dg-danger":""}><b>{balance<0?`${-balance} money short at upkeep`:`${balance} money left after upkeep`}</b></p>
 </section>;
}
