...
export default function FundingPlanSelector({view,command,disabled,onChange}:{view:PlayerView;command:FundedCommand;disabled:boolean;onChange:(command:FundedCommand)=>void}){
 ...
 const options=fundingOptions(view,command.action);
 const payment=(trade:FundedCommand['trades'][number])=>tradeQuote(seat.faction,trade.from,trade.to,trade.amount,factionRulesMode(view))?.input??0;
 ...
 const label=(option:FundedCommand)=>'Pay '+option.trades.map(t=>`${payment(t)} ${t.from}`).join(' + ');
 return <section className="dg-funding" aria-label="Purchase conversion">
  ...
  {options.length>1&&<div className="dg-funding-options" aria-label="Conversion input options">{options.map(option=><button key={JSON.stringify(option.trades)} disabled={disabled} aria-label={label(option.command)} aria-pressed={JSON.stringify(command.trades)===JSON.stringify(option.trades)} onClick={()=>onChange(option.command)}>{option.trades.map(trade=><span key={trade.from}><TradeResourceIcon resource={trade.from}/>{payment(trade)}</span>)}</button>)}</div>}
  <div className="dg-funding-exchange">{command.trades.map(trade=><span key={trade.from} role="img" aria-label={`Spend ${payment(trade)} ${trade.from}`}><TradeResourceIcon resource={trade.from}/><b>−{payment(trade)}</b></span>)}<span aria-hidden="true">→</span><span role="img" aria-label={`Receive ${command.trades.reduce((n,t)=>n+t.amount,0)} ${command.trades[0].to}`}><TradeResourceIcon resource={command.trades[0].to}/><b>+{command.trades.reduce((n,t)=>n+t.amount,0)}</b></span></div>
  <small>{[...new Set(command.trades.flatMap(t=>tradeRates(seat.faction,t.from,t.to,factionRulesMode(view)).map(rate=>`${rate.input} : ${rate.output}`)))].join(' / ')} exchange</small><p className={balance<0?"dg-danger":""}><b>{balance<0?`${-balance} money short at upkeep`:`${balance} money left after upkeep`}</b></p>
 </section>;
}
