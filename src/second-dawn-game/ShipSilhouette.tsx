...
export default function ShipSilhouette({ type, faction }: { type: BlueprintShipType; faction?: FactionId }) {
  ...
  return <svg className="dg-ship-silhouette" role="img" aria-label={`${name} blueprint silhouette`} data-ship-family={family} viewBox="40 0 240 240">
    ...
    <path d={design?.hull ?? HULLS[type]} className="dg-ship-hull" />
    {design && <path d={design.plating} className="dg-ship-plating" />}
    <path d={design?.cockpit ?? 'M160 48 170 88 160 104 150 88Z'} className="dg-ship-cockpit" />
    <path d={design?.engines ?? 'M147 161H173M147 168H173M147 175H173'} className="dg-ship-engine" />
  </svg>;
}
