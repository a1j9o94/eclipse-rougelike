...
export default function ChoiceWorkspace({decision,open,onMinimize,children}:{decision:PendingDecision;open:boolean;onMinimize:()=>void;children:ReactNode}){
 const parentVisible=useContext(DicePresentationVisibilityContext),visible=open&&parentVisible;
 ...
 return <DicePresentationVisibilityContext.Provider value={visible}><div className={`dg-choice-workspace${decision.kind==='exploration'?' dg-choice-placement':''}`} hidden={!open} role="dialog" aria-modal="false" aria-label={`${label[0].toUpperCase()}${label.slice(1)} choice`} onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();onMinimize();}}}>
 ...
 </div></DicePresentationVisibilityContext.Provider>;
}
