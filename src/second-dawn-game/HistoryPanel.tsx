...
export default function HistoryPanel({feed,rollback}:{feed:HistoryFeed;rollback?:HistoryRollbackControl}){
 ...
 <ol>{feed.entries.map(entry=>{const volleys=entry.combatVolleys??(entry.combatVolley?[entry.combatVolley]:[]),presentation=historyPresentationLines(entry);return <li key={entry.revision} className={entry.supersededAtRevision?"dg-history-superseded":undefined}><div className="dg-history-byline"><strong>{entry.actorName}</strong><small>{entry.round===null?'':`Round ${entry.round} · `}#{entry.revision}</small></div><p>{entry.summary}</p>{entry.supersededAtRevision&&<small>Reversed by rollback #{entry.supersededAtRevision}</small>}{rollback?.isHost&&<div className="dg-history-undo"><button disabled={rollback.disabled||!entry.rollbackAvailable||!!entry.supersededAtRevision} onClick={()=>rollback.onSelect(entry)}>Undo to before this action</button>{!entry.rollbackAvailable&&<small className="dg-history-undo-reason">{entry.rollbackUnavailableReason??"This action has no saved rollback checkpoint."}</small>}</div>}{presentation.length>0&&<p className="dg-history-presentation">{presentation.join(' · ')}</p>}{volleys.map((volley,index)=><CombatVolleyResult key={index} volley={volley}/>)} {inspection&&<button className="dg-history-inspect" onClick={()=>inspection.request({kind:'history',entry})}>Inspect public reference</button>}{entry.details.length>0&&<details><summary>Details</summary>{entry.details.map((detail,i)=><p key={i}>{detail}</p>)}</details>}</li>;})}</ol>
