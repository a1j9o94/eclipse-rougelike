...
export default function MovementBattlePreview({view,shipIds,targetSectorId}:Props){
 ...
 return <section className="dg-movement-odds" aria-label="Estimated combat outcome" aria-live="polite">
  {!result?<p>Estimating fleet battle…</p>:result.status==='unavailable'?<p>{result.reason}</p>:<>
   ...
   <details><summary>Estimate assumptions</summary><p>{result.trials} simulations · sampling range {result.lowerPercent}–{result.upperPercent}%. This is an estimate, not a guarantee.</p><p>Current public loadouts and damage, with your fleet {result.defender?'defending':'attacking'}. No voluntary retreats, later reinforcements or upgrades. Automatic hit allocation and tied ship order can differ from player choices.</p><p>Winning means your fleet survives and the enemy fleet is destroyed. It does not predict taking control or destroying population.</p></details>
  </>}
 </section>;
}
