...
const choices = [
 {value:'standard',label:'Standard Eclipse',description:'8 rounds · hidden discoveries and reputation.'},
 {value:'less-random-v1',label:'Régis’s Less Random',description:'10 rounds · open technology · public discoveries and reputation.'},
] as const;
...
export default function RulesModePicker({value,disabled,onChange}:RulesModePickerProps){
 ...
 return <fieldset className="dg-rules-mode" disabled={disabled}>
  ...
  <div className="dg-rules-options">{choices.map(choice=><label key={choice.value} className={value===choice.value?'is-selected':''}>
   <input type="radio" name={id} value={choice.value} checked={value===choice.value} aria-labelledby={`${id}-${choice.value}-label`} aria-describedby={`${id}-${choice.value}-description`} onChange={()=>onChange(choice.value)}/>
  ...
  {value==='less-random-v1'&&<p>1 exploration joker and 5 combat jokers each. Warp portals and Rift Cannons are removed.</p>}
 </fieldset>;
}
