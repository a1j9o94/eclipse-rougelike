  ...
  <section aria-label="Standings" className="dg-score-cards">
   {sorted.map(({score,rank})=>{
    ...
    return <article key={seat.id} className={`dg-score-card${winner?' dg-score-winner':''}`} aria-label={`${faction.name} score`} style={{'--score-faction':seatColor(seat)} as CSSProperties}>
     ...
     <div className="dg-score-contributions" aria-hidden="true">{categories.filter(c=>(c.id!=='reputation'||final)&&score[c.id]>0).map(c=><span key={c.id} style={{flex:score[c.id]/Math.max(1,positiveTotal),background:c.color}} title={`${c.label}: ${score[c.id]} VP`}/>)}</div>
     <div className="dg-score-tokens">{categories.map(c=>{
      const hidden=c.id==='reputation'&&!final,value=score[c.id];
