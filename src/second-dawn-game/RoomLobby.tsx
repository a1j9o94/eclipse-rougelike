...
export function RoomSettingsEditor({settings,disabled,onSave,saveLabel='Save room settings'}:{settings:MultiplayerRoomSettings;disabled:boolean;onSave:(settings:MultiplayerRoomSettings)=>void;saveLabel?:string}){
 ...
 return <section className="dg-room-settings" aria-label="Room settings"><h2>Room settings</h2>
  <div className="dg-room-counts">{(['humanSeatCount','aiCount'] as const).map(key=><div key={key}><span>{key==='humanSeatCount'?'Human players':'Normal AI'}</span><div><button type="button" aria-label={`Fewer ${key==='humanSeatCount'?'human players':'AI opponents'}`} disabled={disabled||draft[key]<=(key==='humanSeatCount'?1:draft.humanSeatCount===1?1:0)} onClick={()=>setCount(key,-1)}>−</button><output>{draft[key]}</output><button type="button" aria-label={`More ${key==='humanSeatCount'?'human players':'AI opponents'}`} disabled={disabled||draft.humanSeatCount+draft.aiCount>=6} onClick={()=>setCount(key,1)}>+</button></div></div>)}</div>
...
export interface RoomLobbyProps {
 ...
 onJoin:(faction:FactionId)=>void;onLeave:()=>void;onFaction:(faction:FactionId)=>void;
...
export default function RoomLobby({lobby,disabled,onJoin,onLeave,onFaction,onReady,onSettings,onStart,onEnter}:RoomLobbyProps){
 ...
 const unavailable:UnavailableFactionColors={};
 for(const seat of lobby.seats)if(seat.occupied&&seat.faction&&seat.slot!==lobby.viewerSlot)unavailable[getFaction(seat.faction).color]=`Chosen by player ${seat.slot}`;
 const available=BASE_FACTIONS.find(f=>!unavailable[f.color])?.id??'terran-directorate';
 const [joiningFaction,setJoiningFaction]=useState<FactionId>(available),[copied,setCopied]=useState(false);
 const selected=own?.faction??(!unavailable[getFaction(joiningFaction).color]?joiningFaction:available);
 ...
 return <main className="dg-room dg-lobby"><div className="dg-room-inner">
  ...
  <section className="dg-room-agreed-rules" aria-label="Agreed room rules"><span><strong>{lobby.settings.humanSeatCount+lobby.settings.aiCount}</strong> players</span>{lobby.settings.humanSeatCount===1?<span><strong>Wait for me</strong> · No solo timer</span>:<><span><strong>{presets.find(p=>p.ms===lobby.settings.timerMs)?.label??`${lobby.settings.timerMs/1000} seconds`}</strong> per turn</span><span>Timeout → Normal AI takeover</span></>}<span>Warp portals {lobby.settings.warpPortals?'enabled':'disabled'}</span></section>
  <div className="dg-room-layout"><aside className="dg-room-roster"><h2>{waiting?'Gather your civilizations':'Room players'}</h2><div role="list" aria-label="Room players">{lobby.seats.map(seat=><article role="listitem" key={seat.slot} className={seat.slot===lobby.viewerSlot?'is-you':''}><span className="dg-room-seat-number">{seat.slot}</span><div><strong>{seat.username&&<span className="dg-room-username">{seat.username} · </span>}{seat.faction?getFaction(seat.faction).name:'Open seat'}</strong><small>{seat.slot===lobby.viewerSlot?'You · ':''}{seat.isHost?'Host · ':''}{seat.occupied?(seat.ready?'Ready':'Choosing faction'):'Waiting for a friend'}</small></div><span className={`dg-room-ready ${seat.ready?'is-ready':''}`} aria-label={seat.ready?'Ready':'Not ready'}>{seat.ready?'✓':'○'}</span></article>)}</div>
   {lobby.settings.aiCount>0&&<p>{lobby.settings.aiCount} Normal AI {lobby.settings.aiCount===1?'opponent':'opponents'} · remaining faction colors assigned at start</p>}
  ...
  </aside><div className="dg-room-faction">{waiting?<><FactionPicker selected={selected} unavailableColors={unavailable} disabled={disabled} onSelect={f=>{setJoiningFaction(f);if(own)onFaction(f);}}/><p className="dg-room-rule">Faction or roster changes clear everyone’s ready state.</p></>:<section className="dg-room-started"><h2>{lobby.status==='finished'?'Final standings await':'The galaxy is underway'}</h2><p>{own?'Your seat and every accepted choice are saved automatically.':'Only the players who joined before the game started can open its board.'}</p></section>}</div></div>
  <footer className="dg-room-footer"><span>{waiting?`${lobby.seats.filter(s=>s.occupied&&s.ready).length} / ${lobby.settings.humanSeatCount} players ready`:'Game saved in this room'}</span>{waiting?own?<><button disabled={disabled} className={own.ready?'':'dg-primary'} onClick={()=>onReady(!own.ready)}>{own.ready?'Not ready':'Ready to play'}</button>{lobby.viewerIsHost&&<button className="dg-primary" disabled={disabled||!canStart} onClick={onStart}>Start room game</button>}</>:<button className="dg-primary" disabled={disabled||!lobby.seats.some(s=>!s.occupied)||Boolean(unavailable[getFaction(selected).color])} onClick={()=>onJoin(selected)}>Join room</button>:own&&<button className="dg-primary" disabled={disabled} onClick={onEnter}>{lobby.status==='finished'?'View final game':'Return to game'}</button>}</footer>
 </div></main>;
}
