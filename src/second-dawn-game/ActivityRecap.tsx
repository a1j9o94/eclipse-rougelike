...
export default function ActivityRecap({baseline,throughRevision,feed,disabled,saving,error,onDismiss}:Props){
 ...
 return <section className="sd-activity-recap" data-through-revision={throughRevision} aria-label="Return to your game">
  ...
  {feed.loading?<p role="status">Loading saved activity…</p>:<><p className="dg-recap-summary" role="status">{entries.length} public {entries.length===1?'action':'actions'} in this return window.</p><ol>{entries.map(entry=><li key={entry.revision}>
   <div><strong>{entry.actorName}</strong><small>{entry.round!==null?`Round ${entry.round}`:''}</small></div>
   <p>{entry.summary}</p>{inspection&&<button className="dg-history-inspect" onClick={()=>inspection.request({kind:'history',entry})}>Inspect public reference</button>}{entry.details.length>0&&<details><summary>Action details</summary>{entry.details.map((detail,index)=><p key={index}>{detail}</p>)}</details>}
  </li>)}</ol></>}
