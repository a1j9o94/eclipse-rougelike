...
export function scoreInspection(view:PlayerView,seatId:string,category:PublicScoreCategory):PublicInspectionDetail {
 ...
 if(frozen)return {title:category.replace(/(^|-)\w/g,word=>word.toUpperCase()),value:score[category]??0,sectorIds:[],contributors:[],explanation:'This eliminated civilization has a frozen public score; current-board contributors are not substituted for that snapshot.'};
 switch(category){
  case 'minorSpecies':return {title:'Minor Species',value:score.minorSpecies??0,sectorIds:[],contributors:(seat.minorSpecies??[]).map(tile=>getMinorSpecies(tile.id).name),explanation:'Retained Minor Species and their public scoring bonuses. Reputation patrons use the number of tiles; their hidden values are never included in this bonus.'};
  ...
  case 'species': {const definition=getFaction(seat.faction);if(definition.special?.ancientPartVp)return {title:'Ancient parts used',value:score.species,sectorIds:[],contributors:[`${seat.ancientPartsUsed??0} Ancient parts used × ${definition.special.ancientPartVp} VP`],explanation:'Each Discovery Tile used as a ship part scores, including parts later removed from your blueprints.'};const scoring=definition.capabilities.endGameVp;const ids=scoring==='controlled-sector'?sectorIds(()=>true):scoring==='surviving-ancient'?view.ships.filter(ship=>ship.type==='ancient').map(ship=>ship.sectorId):[];return {title:'Species bonus',value:score.species,sectorIds:ids,contributors:scoring==='controlled-sector'?labels(ids):scoring==='surviving-ancient'?labels(ids).map(label=>`${label}: Ancient on board`):[],explanation:'This faction-specific public score uses the current board.'};}
  ...
  case 'traitor': return {title:'Traitor penalty',value:score.traitor,sectorIds:[],contributors:[],explanation:getFaction(seat.faction).special?.ignoresTraitorPenalty?'Rho Indi keeps the Traitor tile and cannot form new diplomatic relations, but loses no victory points.':'The current public diplomatic penalty.'};
