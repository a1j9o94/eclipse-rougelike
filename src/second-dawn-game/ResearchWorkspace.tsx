 ...
 return <div className="sd-workspace dg-research-workspace">
  ...
  {selectedTech&&<section ref={detailRef} tabIndex={-1} className="dg-research-local" aria-label={`Research ${selectedTech.name}`}>
   ...
   {selected!==null&&!owned.has(selected)&&(!draftId||draftId===selected)&&<>
    {draft&&<ActionEconomy view={view} action="research" preview={preview}/>} 
    ...
    {draft?.command.type==='trade-and-act'&&<FundingPlanSelector view={view} command={draft.command} disabled={disabled} onChange={command=>onDraft({...draft,command})}/>} 
    {draft&&scienceCost!==null&&<div className="dg-research-commit"><p><strong>{preview?.resourcesAfter.science} science remaining</strong> after acquisition.</p><button className="sd-primary" disabled={disabled||stale||!stillLegal} onClick={()=>onSubmit(draft.command)}>{draft.command.type==='trade-and-act'?'Convert & ':''}Research · {scienceCost} science</button><button onClick={()=>onDraft(null)}>Cancel draft</button>{stale&&<p className="dg-danger">The game changed. Review this research choice before submitting again.</p>}</div>}
   </>}
  </section>}
  <h2 className="dg-market-heading">Available technologies</h2>
  <div className="sd-tech-grid">{['military','grid','nano','rare'].map(track=><section key={track}><h2>{humanize(track)}</h2>{[...new Set(view.technologyMarket)].filter(id=>TECHNOLOGIES.find(t=>t.id===id)?.track===track).map(rawId=>{const id=rawId as TechnologyId;
