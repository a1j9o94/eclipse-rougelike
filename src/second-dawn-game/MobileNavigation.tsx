...
export default function MobileNavigation({selected,onSelect,onActions,pending,onDecision,actionLabel,onBack,onDetails,onConfirm,onEndAction}:{selected:MobileDestination;onSelect:(destination:MobileDestination)=>void;onActions?:()=>void;pending:boolean;onDecision:()=>void;actionLabel?:string;onBack:()=>void;onDetails?:()=>void;onConfirm?:{label:string;disabled:boolean;submit:()=>void};onEndAction?:()=>void}){
 return <footer className="dg-mobile-footer">
  {pending&&<button className="dg-mobile-pending" onClick={onDecision}>Return to decision</button>}
  {actionLabel?<nav className="dg-mobile-action-nav" aria-label="Current action"><button onClick={onBack}>Back</button><span>{actionLabel}</span>{onConfirm?<button className="sd-primary" disabled={onConfirm.disabled} onClick={onConfirm.submit}>{onConfirm.label}</button>:onEndAction?<button onClick={onEndAction}>End action</button>:onDetails&&<button onClick={onDetails}>Details</button>}</nav>:<>
   {onActions&&<button className="dg-mobile-choose-action" onClick={onActions}><StatIcon kind="influence"/>Choose action</button>}
