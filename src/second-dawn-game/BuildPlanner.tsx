...
import { deriveBlueprintStats,effectiveBlueprintParts, type BlueprintShipType } from '../../shared/eclipse/blueprints';
...
export default function BuildPlanner({ view, sectorId, defaultPlacementSectorId=null, disabled, onClose, onSubmit, embedded=false, placementRequest, onPlacementPreview, onLegalTargetsChange }: BuildPlannerProps) {
  ...
  const confirmationReady=!disabled&&!guard.stale&&valid&&total===analysis.limit&&(command.type!=='trade-and-act'||plans.length===1||plans.some(plan=>JSON.stringify(plan.trades)===draft.fundingKey));
  const confirmBuild=()=>{if(!disabled&&!guard.stale&&valid){guard.markSubmitted(command);onSubmit(command);}};
  const content=<>
    ...
    <div className="dg-build-body"><main className="dg-build-workspace"><ActionDraftNotice/>
      <section aria-labelledby="build-capabilities"><h3 id="build-capabilities">1. Choose pieces</h3><div className="dg-build-cards">{BUILD_COMPONENTS.filter(type=>!(own.faction==='exiles'&&type==='starbase')).map(type=>{
        const reason=componentReason(type), blueprint=isShip(type)?own.blueprints.find(candidate=>candidate.shipType===type):null, orbitalBlueprint=type==='orbital'&&own.faction==='exiles'?own.blueprints.find(candidate=>candidate.shipType==='starbase'):null, stats=(blueprint||orbitalBlueprint)?deriveBlueprintStats(own.faction,publicBlueprint((blueprint||orbitalBlueprint)!)):null;
    ...
    <footer className="dg-build-footer"><span>{valid?`${total} ${total===1?'piece':'pieces'} ready · ${new Set(action.builds.map(build=>build.sectorId)).size} deployment ${new Set(action.builds.map(build=>build.sectorId)).size===1?'sector':'sectors'}`:analysis.unplacedCount?`${analysis.unplacedCount} ${analysis.unplacedCount===1?'piece still needs':'pieces still need'} a sector`:'Review the order before building.'}</span><button type="button" onClick={onClose}>Cancel</button><button type="button" className="sd-primary" disabled={disabled||guard.stale||!valid} onClick={confirmBuild}>{command.type==='trade-and-act'?'Convert & ':''}Build {total} {draft.items.every(item=>isShip(item.component))?(total===1?'ship':'ships'):(total===1?'piece':'pieces')} · {analysis.cost} materials</button></footer>
    <ActionConfirmationNotice ready={confirmationReady} noticeKey={JSON.stringify([view.viewerSeatId,view.round,view.actionTurnSerial,command])} title="Build order ready" description={`All ${analysis.limit} build activations are assigned and every piece has a deployment sector. Review the cost, then confirm your order.`} confirmLabel={command.type==='trade-and-act'?'Convert & confirm build':'Confirm build'} onConfirm={()=>{if(confirmationReady)confirmBuild();}}>
      {command.type==='trade-and-act'&&<p>Conversion and construction happen together. {command.trades.map(trade=>`Receive ${trade.amount} ${trade.to} using ${trade.from}`).join('; ')}.</p>}
