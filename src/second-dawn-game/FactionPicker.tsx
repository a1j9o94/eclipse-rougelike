...
import { BASE_FACTIONS, type CivilizationColor, type FactionId } from "../../shared/eclipse/catalog";
...
export default function FactionPicker({ selected, onSelect, disabled = false, unavailableColors = {} }: FactionPickerProps) {
  ...
  const active = BASE_FACTIONS.find(faction => faction.id === selected)!;
  ...
  return <section ref={picker} tabIndex={-1} className="dg-faction-picker" aria-label="Choose civilization">
    <header><div><small>YOUR CIVILIZATION</small><h2>Choose a faction board</h2></div><p>Each color has an alien and Terran side. A color can only be used once.</p></header>
    {compact&&<button type="button" className="dg-faction-jump" onClick={showDetails}>View {active.name} effects</button>}
    <div className="dg-faction-pairs">
      {colors.map(color => {
