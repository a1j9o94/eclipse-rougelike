...
export default function MovementPlanner({view,sourceSectorId,selectedTargetId,disabled,result,onDone,onTargetsChange,onClose,onSubmit,onRoutePreview,onSelectionChange,onChangeSource,showCombatOdds=false}:MovementPlannerProps){
 ...
 return <section className="dg-movement-planner" aria-label="Move fleet">
  ...
  <ActionConfirmationNotice ready={confirmationReady} noticeKey={JSON.stringify([view.viewerSeatId,view.round,view.actionTurnSerial,confirmationCommand])} title="Movement plan ready" description="All move activations are assigned to valid routes. Review the consequences, then confirm your movement." confirmLabel="Confirm move" onConfirm={()=>{if(confirmationReady&&confirmationCommand){draftGuard.markSubmitted(confirmationCommand);onSubmit(confirmationCommand);}}}>
    {!!confirmationPreview?.betrayedPartners.length&&<p className="dg-danger">Breaks diplomacy with {confirmationPreview.betrayedPartners.map(id=>{const partner=view.seats.find(seat=>seat.id===id);return partner?getFaction(partner.faction).name:id;}).join(', ')}. You take the traitor card ({getFaction(own.faction).special?.ignoresTraitorPenalty?'0':'−2'} VP), return these ambassadors, and cannot form new relations while holding it.</p>}
    <ActionEconomy view={view} action="move" preview={confirmationPreview}/>
