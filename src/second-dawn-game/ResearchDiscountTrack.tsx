...
export default function ResearchDiscountTrack({track,count}:{track:TechnologyTrack;count:number}){
 ...
 return <div className="dg-research-discounts" role="group" aria-label={`${name} research discounts`}>
  <p><strong>{full?'Track full · no research slots remaining':`Current discount: ${RESEARCH_DISCOUNTS[count]} science`}</strong></p>
  <ol aria-label="Science discount progression">{RESEARCH_DISCOUNTS.map((discount,index)=><li key={index} aria-current={!full&&count===index?'step':undefined} aria-label={`${index} researched: ${discount} science discount${!full&&count===index?', current':''}`} className={index<count?'is-used':undefined}><strong>{discount?`−${discount}`:'0'}</strong><small>{!full&&count===index?'Now':`${index+1}/7`}</small></li>)}</ol>
  {!full&&<p className="dg-discount-next">{count+1<RESEARCH_TRACK_CAPACITY?`After next research: ${RESEARCH_DISCOUNTS[count+1]} science discount`:'Next research fills this track'}</p>}
 </div>;
}
