...
export default function TradePanel({view,candidates,disabled,onSubmit}:TradePanelProps) {
 ...
 const maximum=Math.floor(seat.resources[from]/ratio);
 const result=tradeResources(seat.resources,seat.faction,from,to,amount);
 ...
 return <section className="dg-trade-panel" aria-label="Trade resources">
  ...
  <div className="dg-trade-swap" aria-live="polite"><div role="img" aria-label={`Spend ${amount*ratio} ${from}`}><TradeResourceIcon resource={from}/><strong>−{amount*ratio}</strong><small>{from}</small></div><span aria-hidden="true">→</span><div role="img" aria-label={`Receive ${amount} ${to}`}><TradeResourceIcon resource={to}/><strong>+{amount}</strong><small>{to}</small></div></div>
  <div className="dg-trade-balance" aria-label="Resources after trade">{RESOURCES.map(resource=><span key={resource}><TradeResourceIcon resource={resource}/>{title(resource)}: {seat.resources[resource]} → {result.ok?result.resources[resource]:seat.resources[resource]}</span>)}</div>
  {!legalPair?<p role="status">No affordable trade is available now.</p>:!result.ok?<p role="status">{result.message}</p>:null}
  <button className="sd-button primary dg-trade-confirm" type="button" disabled={!canSubmit} onClick={()=>{if(canSubmit)onSubmit({type:'trade',from,to,amount});}}>Confirm trade</button>
 </section>;
}
