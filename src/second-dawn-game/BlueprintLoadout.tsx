...
export default function BlueprintLoadout({faction,blueprint}:{faction:FactionId;blueprint:ShipBlueprint}){
 ...
 return <div className="dg-readonly-loadout" role="group" aria-label={`${name} installed loadout`}>
  ...
  {blueprint.outsideParts.length>0&&<div className="dg-outside-loadout" role="group" aria-label="Outside-grid parts">{blueprint.outsideParts.map((id,index)=><div className="dg-readonly-part dg-readonly-overlay" key={`${id}:${index}`}><small>Outside grid</small><strong>{getShipPart(id).name}</strong><ShipPartStats partId={id}/></div>)}</div>}
...
export function ShipCapabilities({stats,neutral=false}:{stats:ShipStats;neutral?:boolean}){
 return <div className="dg-ship-capabilities" role="group" aria-label="Blueprint totals">
  ...
  {neutral&&stats.weapons.map((weapon,index)=><StatBadge key={index} icon={weapon.kind} value={`${weapon.dice} × ${weapon.damage}`} label="dice × damage" explanation={`Roll ${weapon.dice} ${weapon.color} ${weapon.kind} dice; each hit deals ${weapon.damage} damage.`} color={{yellow:'#f0d477',orange:'#ffb076',blue:'#91c9ff',red:'#ff9391'}[weapon.color]}/>)}
 </div>;
}
