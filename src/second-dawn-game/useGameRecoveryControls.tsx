...
export function useGameRecoveryControls({credential,matchId,view,connected,busy,onHome,onRoom}:Props){
 const recoverCheckpoint=useAction(api.eclipseHistoryRecovery.recoverCheckpoint);
 ...
 const historyRollback={isHost:rollback?.isHost??false,disabled:disabled||!!blockedReason,onSelect:(entry:PublicHistoryEntry)=>{setError('');setTarget(entry);setMenuOpen(false);if(!entry.rollbackAvailable&&entry.rollbackRecoverable)void perform(async stillCurrent=>{if(!credential||!matchId)return;const result=await recoverCheckpoint({credential,matchId,targetRevision:entry.revision});if(!stillCurrent())return;if(!result.ok)throw new Error(result.reason);setTarget(current=>current?.revision===entry.revision?{...current,rollbackAvailable:true}:current);});}};
