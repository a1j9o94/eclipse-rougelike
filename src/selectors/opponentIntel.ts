...
export type MpOpponentBasics = {
  ...
  roomDetails?: { room?: { gameConfig?: { startingShips?: number } }; players?: Array<{ playerId:string; playerName?:string }> } | null
}
...
export function selectOpponentIntel(gameMode: 'single'|'multiplayer', multi?: MpOpponentBasics): {
  ...
  try {
    ...
    const opponentName = opp.playerName || (multi.roomDetails?.players || []).find(p=>p.playerId===opp.playerId)?.playerName
    const last = getLastSeenFleet(opp.playerId)
    if (last && last.length > 0) return { source: 'last_combat', fleet: last as Ship[], round, opponentName }
    ...
    if (snapFleet.length > 0) {
      ...
      return { source: 'default', fleet: ships, round, opponentName }
    }
    ...
    if (ids.length > 0 && starting > 0) {
      ...
      return { source: 'default', fleet: ships as Ship[], round, opponentName }
    }
    return { source: 'default', fleet: null, round, opponentName }
  } catch {
