...
for(const [name,engine] of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  assert.equal(await wheel(false),false);assert.equal(await zoom(),'135%');
  ...
  assert.deepEqual(errors,[]);console.log(`${name}: map-only ctrl-wheel zoom, ordinary scrolling, Safari gesture fallback and duplicate suppression passed.`);
