...
try{
 ...
 await page.goto('http://127.0.0.1:5175/?position=workflow-trade#second-dawn-preview');await page.getByRole('button',{name:'Empire',exact:true}).click();await page.getByRole('button',{name:'Trade resources',exact:true}).click();
 const panel=page.getByRole('region',{name:'Trade resources',exact:true});await panel.waitFor();await page.screenshot({path:`${dir}/${width}-trade.png`});
 ...
 await page.getByRole('button',{name:'Confirm trade',exact:true}).click();await page.waitForFunction(expected=>Number(document.querySelectorAll('.dg-mobile-resources>span strong')[1]?.textContent)===expected,science+1);
