...
for(const [width,height,enlarged]of[[390,844,true],[844,390,false]])for(const screen of['research','trade']){
 ...
 await page.goto('http://127.0.0.1:5175/?position=midgame#second-dawn-preview');await page.getByRole('button',{name:'Empire',exact:true}).click();await page.getByRole('button',{name:screen==='research'?'Research technologies':'Convert resources',exact:true}).click();
