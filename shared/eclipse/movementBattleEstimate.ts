...
export async function movementBattleEstimate(view:PlayerView,shipIds:readonly string[],targetSectorId:string,signal?:AbortSignal):Promise<MovementBattleEstimate>{
 ...
 if(friends.length>selected.length&&target.owner!==own.id&&target.owner!==enemies[0].owner)return unavailable('Reinforcements in this contested sector need the final arrival order.');
 ...
 const defender=target.owner===own.id&&!enemies.some(neutral);
