...
export function scoreSeat(state: GameState, seat: Seat): ScoreBreakdown {
  ...
  return calculateScore({ playerId: seat.id, faction: seat.faction, reputation: hidden?.reputation ?? [], ambassadors: seat.ambassadors.length,
    sectors: state.sectors.filter(sector => sector.owner === seat.id).map(sector => ({ id: sector.id, printedVp: requireSectorDefinition(Number(sector.tileId)).victoryPoints, monoliths: Number(sector.monolith), portalVp: sector.portalVp ?? 0, orbitalPopulated: sector.orbital && sector.population.some(cube => cube.squareId === 'orbital'), shrines:(seat.shrines??[]).filter(shrine=>shrine.sectorId===sector.id).length })),
