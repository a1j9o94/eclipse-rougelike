...
export function validRuleOptions(options:GameRuleOptions|undefined):boolean {
 ...
 return Object.entries(options).every(([key,value])=>key==='roundLimit'||(['openTechnology','publicDiscoveries','publicReputation','explorationRules','combatJokers','technologyVariant','discoveryVariant','factionVariant','passOrderTurnOrder'].includes(key)&&typeof value==='boolean'));
}
